// Fun with Arrays & ES6
const suits=['major','swords','wands','coins','cups'];
...
function makeDeck(){
    ...
    function makeCard(suit,rank,idx){
        let obj={};
        obj['name']= (suit==='major') ? rank : rank +" of "+ suit;
        obj['suit']=suit;
        obj['rank']=idx;
        return obj;
    }
...
function tellFortune(hand) {
    // console.log(descriptions.interpret[0]);
    let cards=[];
    hand.forEach(function (ele) {
        console.log(
            deck[ele].name.toUpperCase().split(),
            descriptions.interpret[ele].keywords,
            descriptions.interpret[ele].fortune_telling[getRandom(descriptions.interpret[ele].fortune_telling.length-1)].split()

        );
    });
}

function getRandom(max=deck.length-1) {
